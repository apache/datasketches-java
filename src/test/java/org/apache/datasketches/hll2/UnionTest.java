...
public class UnionTest {
  ...
  public void checkConversions() {
    final int lgK = 4;
    final HllSketch sk1 = new HllSketch(lgK, TgtHllType.HLL_8);
    final HllSketch sk2 = new HllSketch(lgK, TgtHllType.HLL_8);
    final int u = 1 << 20;
    for (int i = 0; i < u; i++) {
      sk1.update(i);
      sk2.update(i + u);
    ...
    final HllSketch rsk2 = union.getResult(TgtHllType.HLL_6);
    final HllSketch rsk3 = union.getResult(TgtHllType.HLL_4);
    final double est1 = rsk1.getEstimate();
    final double est2 = rsk2.getEstimate();
    final double est3 = rsk3.getEstimate();
    //println("Est1: " + est1);
    //println("Est2: " + est2);
    //println("Est3: " + est3);
  ...
  public void checkUnionHeapifyRebuildAfterMerge() {
    ...
    final int u = 1 << (lgK - 3); //(lgK < 8) ? 16 : 1 << (lgK - 3) //allows changing lgK above
    ...
    final HllSketch sk3 = HllSketch.heapify(wseg); //rebuilds sk3
  ...
  public void druidUseCase() {
   ...
   final double err = (est / trueCount) - 1.0;
   final double rse3 = (3 * 1.04)/Math.sqrt(1 << lgK);
   println(err + " < " + rse3);
   assertTrue(err < rse3);
  }
