...
import static org.apache.datasketches.kll2.KllPreambleUtil.DATA_START_ADR;
import static org.apache.datasketches.kll2.KllPreambleUtil.DATA_START_ADR_SINGLE_ITEM;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentEmptyFlag;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentFamilyID;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentFlags;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentK;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentLevelZeroSortedFlag;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentM;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentMinK;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentN;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentNumLevels;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentPreInts;
import static org.apache.datasketches.kll2.KllPreambleUtil.getMemorySegmentSerVer;
 ...
 *
 */
final class KllMemorSegmentValidate {
  final Memory srcMem;
  ...
  KllMemorSegmentValidate(final Memory srcMem, final SketchType sketchType) {
    this(srcMem, sketchType, null);
  }

  KllMemorSegmentValidate(final Memory srcMem, final SketchType sketchType, final ArrayOfItemsSerDe<?> serDe) {
    final long memCapBytes = srcMem.getCapacity();
    if (memCapBytes < 8) { throw new SketchesArgumentException(MEMORY_TOO_SMALL + memCapBytes); }
    this.srcMem = srcMem;
    this.sketchType = sketchType;
    this.serDe = serDe;
    preInts = getMemorySegmentPreInts(srcMem);
    serVer = getMemorySegmentSerVer(srcMem);
    sketchStructure = SketchStructure.getSketchStructure(preInts, serVer);
    familyID = getMemorySegmentFamilyID(srcMem);
    if (familyID != Family.KLL.getID()) { throw new SketchesArgumentException(SRC_NOT_KLL + familyID); }
    flags = getMemorySegmentFlags(srcMem);
    k = getMemorySegmentK(srcMem);
    m = getMemorySegmentM(srcMem);
    KllHelper.checkM(m);
    KllHelper.checkK(k, m);
    //flags
    emptyFlag = getMemorySegmentEmptyFlag(srcMem);
    level0SortedFlag  = getMemorySegmentLevelZeroSortedFlag(srcMem);
  ...
  private void validate() {
    ...
    switch (sketchStructure) {
      case COMPACT_FULL: {
        ...
        n = getMemorySegmentN(srcMem);
        //if (n <= 1) { memoryValidateThrow(N_AND_COMPACT_FULL); } //very old sketches prior to serVer=2 will violate.
        minK = getMemorySegmentMinK(srcMem);
        numLevels = getMemorySegmentNumLevels(srcMem);
