...
class KllDirectDoublesSketch extends KllDoublesSketch {
  ...
  KllDirectDoublesSketch(
      ...
      final KllMemorSegmentValidate memVal) {
  ...
  static KllDirectDoublesSketch newDirectUpdatableInstance(
      ...
      final MemoryRequestServer memReqSvr) {
    setMemorySegmentPreInts(dstMem, UPDATABLE.getPreInts());
    setMemorySegmentSerVer(dstMem, UPDATABLE.getSerVer());
    setMemorySegmentFamilyID(dstMem, Family.KLL.getID());
    setMemorySegmentK(dstMem, k);
    setMemorySegmentM(dstMem, m);
    setMemorySegmentN(dstMem, 0);
    setMemorySegmentMinK(dstMem, k);
    setMemorySegmentNumLevels(dstMem, 1);
    ...
    final KllMemorSegmentValidate memVal = new KllMemorSegmentValidate(dstMem, DOUBLES_SKETCH, null);
  ...
  public long getN() {
    ...
    else { return getMemorySegmentN(wmem); }
  }
  ...
  @Override
  int getMinK() {
    if (sketchStructure == COMPACT_FULL || sketchStructure == UPDATABLE) { return getMemorySegmentMinK(wmem); }
    return getK();
  }
  ...
  void incN(final int increment) {
    if (readOnly) { throw new SketchesArgumentException(TGT_IS_READ_ONLY_MSG); }
    setMemorySegmentN(wmem, getMemorySegmentN(wmem) + increment);
  }

  @Override
  void incNumLevels() {
    if (readOnly) { throw new SketchesArgumentException(TGT_IS_READ_ONLY_MSG); }
    int numLevels = getMemorySegmentNumLevels(wmem);
    setMemorySegmentNumLevels(wmem, ++numLevels);
  }

  @Override
  boolean isLevelZeroSorted() {
    return getMemorySegmentLevelZeroSortedFlag(wmem);
  }
  ...
  void setLevelZeroSorted(final boolean sorted) {
    if (readOnly) { throw new SketchesArgumentException(TGT_IS_READ_ONLY_MSG); }
    setMemorySegmentLevelZeroSortedFlag(wmem, sorted);
  }

  @Override
  void setMinK(final int minK) {
    if (readOnly) { throw new SketchesArgumentException(TGT_IS_READ_ONLY_MSG); }
    setMemorySegmentMinK(wmem, minK);
  }

  @Override
  void setN(final long n) {
    if (readOnly) { throw new SketchesArgumentException(TGT_IS_READ_ONLY_MSG); }
    setMemorySegmentN(wmem, n);
  }
