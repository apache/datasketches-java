...
import static org.apache.datasketches.common.SpecialValueLayouts.JAVA_DOUBLE_UNALIGNED_BIG_ENDIAN;
import static org.apache.datasketches.common.SpecialValueLayouts.JAVA_FLOAT_UNALIGNED_BIG_ENDIAN;
...
public final class TDigestDouble {
  ...
  public byte[] toByteArray() {
    ...
    posSeg.setByte((byte) getPreambleLongs());
    posSeg.setByte(SERIAL_VERSION);
    ...
    if (isSingleValue()) {
      posSeg.setDouble(minValue_);
      return bytes;
    }
    posSeg.setInt(numCentroids_);
    posSeg.setInt(0); // unused
    posSeg.setDouble(minValue_);
    posSeg.setDouble(maxValue_);
    for (int i = 0; i < numCentroids_; i++) {
      posSeg.setDouble(centroidMeans_[i]);
      posSeg.setLong(centroidWeights_[i]);
    }
    return bytes;
  }
   ...
   * Supports reading format of the reference implementation (auto-detected).
   * @param seg instance of MemorySegment
   * @return an instance of TDigest
   */
  public static TDigestDouble heapify(final MemorySegment seg) {
    return heapify(seg, false);
  }

  /**
   * Deserialize TDigest from a given MemorySegment. Supports reading compact format
  ...
  public static TDigestDouble heapify(final MemorySegment seg, final boolean isFloat) {
    ...
    final byte sketchType = posSeg.getByte();

    if (sketchType != (byte) Family.TDIGEST.getID()) {
      if ((preambleLongs == 0) && (serialVersion == 0) && (sketchType == 0)) { return heapifyCompat(seg); }
  ...
  private static TDigestDouble heapifyCompat(final MemorySegment seg) {
    int offset = 0;
    final int type = seg.get(JAVA_INT_UNALIGNED_BIG_ENDIAN, offset); offset += Integer.BYTES;
