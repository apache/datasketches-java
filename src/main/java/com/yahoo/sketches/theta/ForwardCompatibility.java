...
/**
 * Used to convert older serialization versions 1 and 2 to version 3.  Note that the serialization
 * version is different from the Sketch Library version.
 * 
 * @author Lee Rhodes
 */
class ForwardCompatibility {
   ...
   */
  static final Sketch heapify1to3(Memory srcMem, long seed) {
   ...
   * @return a SerVer 3 HeapCompactOrderedSketch
   */
  static final Sketch heapify2to3(Memory srcMem, long seed) {
    int memCap = (int) srcMem.getCapacity();
    assert(isMultipleOf8AndGT0(memCap));
    
    short seedHash = computeSeedHash(seed);
    short memSeedHash = srcMem.getShort(SEED_HASH_SHORT);
    checkSeedHashes(seedHash, memSeedHash);
